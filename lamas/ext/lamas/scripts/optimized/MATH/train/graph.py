...
class MATHGraph:
    ...
    async def __call__(self, problem: str, log_path: str = None):
        ...
        # Initial programmer solution (always executed in training)
        code_solution = await self.programmer(problem=problem, return_usage=True)
        refined_solution = await self.custom(input=problem + f"\nCode output: {code_solution['output']}", instruction=prompt_custom.REFINE_ANSWER_PROMPT, return_usage=True)

        # Track initial programmer call
        if "Programmer" not in operator_iterations:
            operator_iterations["Programmer"] = []
        operator_iterations["Programmer"].append(1)
        ...
        for layer_idx, selected_names in enumerate(selected_names_layers):
            ...
            if self.parallel_execution:
                ...
                # Track layer token counts in order for critical path tracking
                layer_token_counts = []
                ...
                for op_name in selected_names:
                    ...
                    if op_name == "MultiGenerateCoT":
                        ...
                        # For Programmer: add virtual tokens for tool execution time + refined solution tokens
                        ...
                        cp_token_count = raw_tokens + refined_tokens + tool_exec_time * 50.0  # 50 tokens/sec as virtual cost
                    else:
